...
## Downloadthe data and read into R
...
We use random forest since this is a classifdaction problem, but do not use the train function from the caret package since this takes very much longer to compute on the author's machine.
...
This is very high accuracy, so we have confidence in our model, but overfitting may be occurring and this process still takes several minutes, so we attempt now to assess the relative improtance of the predictors, and reduce them in number, if possible, using only those that have high predictive value. This may result in a reduction in accuracy within the training data, but an improvement in out of sample prediction accuracy.
...
These methods are introduced and explained in the Edx MOOC The Analytics Edge, which is where the author came across them.
...
Here we measure the importance of a variable by averaging the reduction in impurity of each leaf, taking the average over all the times that variable is selected for splitting in all of the trees in the forest. The greater the reduction in impurity, the more important the variable.
...
sum(diag(ctClasserf))/sum(ctClasserf)
```
This also achieves almost perfect accuracy, and we have more confidence that it will perform well on out of sample data.
